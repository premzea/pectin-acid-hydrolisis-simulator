...
#figure(
  align(center)[#table(
    columns: (43.06%),
    align: (left,),
    table.header([\#\# 2. The Core Distinction: Forward vs.~Inverse
      Problems To design a self-learning system, one must master the
      mathematical asymmetry between moving from causes to effects,
      versus moving from effects back to causes. \#\# Mathematical
      Definitions],),
    table.hline(),
    [\#\# 3. Mechanistic Process Modeling\#\# Anatomy of a Mechanistic
    Model Unlike empirical black-box equations (such as polynomials,
    random forests, or deep neural networks) which simply fit lines to
    arbitrary shapes, a mechanistic model derives its mathematical form
    directly from physical constraints and structural hypotheses. A
    standard continuous-time mechanistic process model is formalized as
    a system of parameterized ordinary differential equations:],
    ...
    [Where:],
    [\* $upright(bold(w))\(t\)in bb(R)^(n_w)$ is the vector of internal
    system states (e.g., true, unobserved physical concentrations inside
    the reactor phase). \* $upright(bold(x))\(t\)in bb(R)^(n_x)$ is the
    vector of inputs or control variables manipulated by the
    investigator (e.g., setpoint temperature $T$, acid concentration or
    $upright("pH")$, solid-to-liquid ratio, raw material particle size
    $d_p$). \* $bold(theta) in bb(R)^(n_theta)$ is the vector of
    invariant physical parameters (e.g., pre-exponential frequency
    factors, activation energies, mass transfer coefficients). \*
    $upright(bold(y))\(t\)in bb(R)^(n_y)$ is the vector of observables
    or measurements generated by instruments. Crucially,
    $upright(bold(y))$ is often a lower-dimensional projection of
    $upright(bold(w))$ corrupted by noise, as we rarely measure every
    internal state directly. \* $upright(bold(f))$ represents the
    structural transition vector field dictated by conservation laws. \*
    $upright(bold(g))$ represents the measurement mapping function.],
    [\#\# The Pectin 4-Pool Kinetic Differential Equations Applying this
    formal structure to our acid-hydrolysis cascade, we define our four
    states as mass fractions normalized to the total initial insoluble
    protopectin available in the peel matrix:],
    [$ upright(bold(w))\(t\)= mat(delim: "[", P_(upright("matrix"))\(t\); P_(upright("sol"))\(t\); P_(upright("lowMW"))\(t\); P_(upright("loss"))\(t\)) $],
    [The governing system of non-linear differential equations is
    formulated as:],
    ...
    [The kinetics are explicitly driven by environmental inputs. The
    rate constants follow a modified Arrhenius formulation to capture
    the non-linear impacts of Temperature ($T$, in Kelvin) and chemical
    ...
    [Where $R$ is the universal gas constant, $A_i$ are the
    pre-exponential frequency factors, $E_(a\,i)$ are the activation
    energies, and $alpha\,beta\,gamma$ are scaling exponents
    representing mass transfer and acid-catalysis intensities. \#\# The
    Value of Lumped, Abstracted Frameworks Real passion-fruit pectin
    does not exist as three neat, isolated molecular categories. It is a
    highly heterogeneous polydisperse polymer network containing
    hundreds of distinct chain lengths, branching degrees, and
    esterification patterns. Our 4-pool model is fundamentally an
    abstracted lumped model. It condenses an infinite-dimensional
    polymer molecular weight distribution into four discrete operational
    "compartments." Despite being molecularly incomplete, lumped models
    are remarkably powerful tools for process engineering. They avoid
    the computational intractability and over-parameterization of
    tracking thousands of individual polymer lengths. Instead, they
    focus entirely on macroscopic engineering metrics. By capturing the
    primary, dominant pathways of mass transport and chemical
    transformation, they remain highly predictive for industrial
    control, provided their parameters are rigorously identified. \#\#
    Physical Constraints and Mathematical Invariants A primary advantage
    of mechanistic formulations over black-box models is their intrinsic
    adherence to physical laws. This provides stability when
    extrapolating to unmeasured operating conditions. \#\# 1.
    Conservation Invariants Because the 4-pool differential equations
    describe a closed mass cascade where mass leaving one pool must
    immediately enter the next, the total mass is conserved perfectly at
    every point in time. Summing the vector field yields:],
    ...
    [If a machine learning algorithm predicts a total mass sum of $1.05$
    or $0.92$, it violates the first law of thermodynamics. The
    mechanistic model renders such physical impossibilities
    mathematically impossible. \#\# 2. Strict State Positivity Because
    concentrations cannot be negative, the model's equations naturally
    enforce a lower bound at zero. For any pool $w_i$, its outflow rate
    is directly proportional to its current concentration ($w_i$). As
    any pool approaches zero, its evacuation rate drops smoothly to
    zero:],
    [$ lim_(w_i arrow.r 0^(+)) frac(d w_i, d t) gt.eq 0 $],
    [This mathematical structure guarantees that if the system
    initializes with non-negative mass
    ($upright(bold(w))\(0\)gt.eq upright(bold(0))$), all states remain
    strictly positive ($upright(bold(w))\(t\)in\[0\,1\]^4$) across all
    time horizons $t in\[0\,oo\)$. Empirical models like polynomials
    frequently dip into unphysical negative predictions when
    extrapolating into harsh processing regions.],
  )]
  , kind: table
  )
...
------------------------------ \#\# 5. Identifiability Before attempting
parameter estimation, we must determine if our parameters can actually
be recovered from the data we plan to collect. This is the study of
identifiability. \#\# Structural vs.~Practical Identifiability
...
sensitivity. ------------------------------ \#\# 6. Experimental Design
as an Information Problem Data shouldn't be collected passively. We must
explicitly design experiments to maximize the information content of our
Fisher Information Matrix. \#\# Classical vs.~Information-Based Designs
...
#figure(
  align(center)[#table(
    columns: (43.06%),
    align: (left,),
    table.header([\#\# 7. Bayesian Inference from First Principles When
      dealing with sparse, noisy data, single-point parameter estimates
      (like Maximum Likelihood Estimation) are insufficient. They offer
      no insight into parameter uncertainty or correlations. We use
      Bayesian inference to treat parameters as probability
      distributions rather than fixed points. \#\# The Foundational
      Mechanics Bayesian inference updates our prior beliefs about a
      system after confronting them with real-world data. It is governed
      by Bayes' Theorem:],),
    table.hline(),
    [\#\# 8. Proxy and Measurement Calibration Models\#\# The
    Measurement Bottleneck To update our model parameters, the inference
    engine requires data for the individual states
    ($upright("P")_(upright("sol"))$,
    ...
    labor and material. They cannot provide the rapid feedback needed
    for automated, real-time optimization. \#\# The Proxy Solution To
    accelerate the loop, we introduce proxy measurements: cheap,
    non-destructive, instantaneous indicators that can be monitored
    continuously online (e.g., Fourier-Transform Infrared Spectroscopy
    (FTIR), Total Soluble Solids via Refractive Index
    ($""^compose upright("Brix")$), online fluid density, or simple
    medium $upright("pH")$).],
    [┌────────────────────────┐ ┌────────────────────────┐ │ TRUE METRIC
    ...
    └────────────────────────┘ Calibration └────────────────────────┘
    Model],
    [\#\# Mathematical Calibration Mappings We establish a mathematical
    calibration model (such as Partial Least Squares Regression \[PLS\]
    or a Gaussian Process) to infer the true state vector
    $upright(bold(w))$ from the raw proxy spectra or sensor arrays
    $upright(bold(z))$:],
    [$ upright(bold(w))_(upright("estimated")) = cal(M)_(upright("calib"))\(upright(bold(z))\)+ bold(epsilon.alt)_(upright("proxy")) $],
    [For our pectin project, an FTIR spectrometer measures absorbance
    across a range of wavenumbers
    $upright(bold(z)) =\[A_(nu_1)\,A_(nu_2)\,dots.h\,A_(nu_P)\]$. The
    calibration model isolates the fingerprint region for galacturonic
    acid ester linkages ($1740 upright(" cm")^(- 1)$ and
    $1630 upright(" cm")^(- 1)$) to predict real-time concentrations:],
    [$ hat(P)_(upright("sol"))\(t\)= beta_0 + sum_(j = 1)^P beta_j dot.op A_(nu_j)\(t\) $],
    [\#\# Compounding Uncertainty Analysis A common mistake is treating
    proxy estimates as absolute truth, ignoring the calibration error
    $bold(epsilon.alt)_(upright("proxy"))$. If a proxy model has an
    $R^2 = 0.90$ with a root-mean-square error
    ...
    [By adding the proxy calibration error to the overall noise budget,
    the Bayesian framework automatically dampens overconfidence. It
    recognizes that proxy data are indirect, adjusting the posterior
    distributions to reflect this uncertainty until confirmed by slower,
    more authoritative offline analysis.],
  )]
  , kind: table
  )
...
#figure(
  align(center)[#table(
    columns: (43.06%),
    align: (auto,),
    [\#\# 10. Uncertainty-Aware Optimization and Closed-Loop Active
    Learning Once the Bayesian parameters and model discrepancies are
    mapped, the system can autonomously guide its next steps using
    active learning. \#\# The Optimization Objective Our engineering
    goal is to find the optimal operating inputs
    ...
    [\#\# Machine Learning Surrogates (Gaussian Processes) Evaluating
    our complex system of ODEs and propagating full Bayesian parameter
    distributions for thousands of candidate designs during optimization
    is computationally expensive. To accelerate this process, we
    construct a Gaussian Process (GP) Surrogate Model. The GP directly
    learns the mapping from inputs $upright(bold(x))$ to the
    optimization objective $f\(upright(bold(x))\)$, drawing on the
    historical database of past physical runs. Crucially, a GP does not
    just predict a single value; it provides a full Gaussian probability
    distribution for its predictions at any point in the input space,
    defined by a mean $mu\(upright(bold(x))\)$ and an explicit variance
    ...
    strategies:],
    [\* Exploitation: Sampling in regions where the model predicts a
    high mean outcome ($mu\(upright(bold(x))\)$ is maximized),
    fine-tuning known high-performing areas. \* Exploration: Sampling in
    regions where the model's uncertainty is massive
    ($sigma\(upright(bold(x))\)$ is maximized), exploring unknown
    territory to discover new opportunities or correct model blind
    spots.],
    [We manage this balance using an Acquisition Function
    ($alpha\(upright(bold(x))\)$). The optimization algorithm evaluates
    this function across thousands of candidate designs, selecting the
    single point where $alpha\(upright(bold(x))\)$ is maximized as the
    next experiment. \#\# 1. Upper Confidence Bound (UCB)],
    ...
    [\* Setting a small $kappa$ forces the system to prioritize
    exploitation (high $mu$). \* Setting a large $kappa$ forces the
    system to prioritize exploration (high $sigma$).],
    [\#\# 2. Expected Improvement (EI) The Expected Improvement
    criterion calculates the expectation of improving over the current
    best-known experimental result $f\(upright(bold(x))^(+)\)$:],
    ...
    [Where $Phi\(dot.op\)$ and $phi.alt\(dot.op\)$ are the standard
    normal cumulative distribution and probability density functions,
    and $Z$ is defined as:],
...
```
    ...
    [\#\# The Autonomous Closed Loop in Practice When operating fully
    closed-loop, the self-learning framework executes these steps
    autonomously:],
    [1. Evaluate: The GP surrogate evaluates the Expected Improvement
    $alpha_(upright("EI"))\(upright(bold(x))\)$ across a vast grid of
    candidate operating conditions. 2. Select: It identifies the optimal
    next experiment $upright(bold(x))_(upright("next"))$ that maximizes
    the acquisition function. 3. Execute: The automated laboratory
    platform adjusts the process controls to matches
    $upright(bold(x))_(upright("next"))$, running the physical
    extraction. 4. Monitor: Online proxy sensors (such as FTIR or
    refractive index) track the reaction progress in real time. 5.
    Infer: The MCMC engine processes the new data footprint, updating
    the joint posterior parameter distributions
    $p\(bold(theta) divides D\)$ and re-calibrating the discrepancy
    function $delta\(x\)$. 6. Repeat: The GP surrogate updates its
    predictions based on the new knowledge, and the loop repeats until
    the optimization target is reached or the uncertainty collapses
